...
class planner_node(Node):
    def __init__(self, odom_raw_data, lidar_raw_data):
        super().__init__('path_node')

        self.lidar = lidar_raw_data
        self.odom = odom_raw_data

        self.wait_for_lidar()

        end_of_robot_position = [40, 25]

        self.grid_map_obj = grid_map()
        self.pixellate_map_obj = pixellate_map()

        pl.figure(figsize=(20, 10))
        pl.subplot(1, 3, 1)
        position = [self.odom.pose[0], self.odom.pose[1], self.odom.pose[2]]
        self.grid_map_obj.iterateLidar( self.lidar.lidar_buf.tolist(), position)
        self.grid_map_obj.printMap(position)
        
        pl.subplot(1, 3, 2)
        self.pixellate_map_obj.prob_map = self.grid_map_obj.prob_map
        self.pixellate_map_obj.pixellate_map()
        self.pixellate_map_obj.print_drive_map()

        ax = pl.subplot(1, 3, 3)
        ax.set_xlim(0, self.pixellate_map_obj.numberOfBox-1)
        ax.set_ylim(0, self.pixellate_map_obj.numberOfBox-1)

        self.pixellate_map_obj.calculate_dists()
        self.pixellate_map_obj.print_path_map()
        ...
        while(True):
            ...
            position = [self.odom.pose[0], self.odom.pose[1], self.odom.pose[2]]        
            self.grid_map_obj.iterateLidar( self.lidar.lidar_buf.tolist(), position)
            self.grid_map_obj.update(position)

            self.pixellate_map_obj.prob_map = grid_map_obj.prob_map
            self.pixellate_map_obj.pixellate_map()
            self.pixellate_map_obj.update_drive_map()

            position = [self.odom.pose[0], self.odom.pose[1], self.odom.pose[2]]
            self.pixellate_map_obj.clear_drive_map()
            self.pixellate_map_obj.set_end_position(end_of_robot_position)

            current_pos = box_coord(position[0], position[1], grid_map_obj)
            print("Robot BOX position ", [position[0], position[1]])
            self.pixellate_map_obj.set_start_position([current_pos[0], current_pos[1]])
            print("Robot position ", [current_pos[0], current_pos[1]])
            

            self.pixellate_map_obj.calculate_dists()
            self.pixellate_map_obj.update_path_map()
            ...
            self.pixellate_map_obj.path_finding()
            self.pixellate_map_obj.draw_path()  
            print("Robot path  ",self.pixellate_map_obj.path)



    def wait_for_lidar(self):
        rate = self.lidar.create_rate(8)
    
        while self.lidar.lidar_buf == 0:
            rate.sleep()
...
#    pixellate_map_obj = pixellate_map()
    ...
    path_node = planner_node(odom_raw_data, laser_raw_data)
    executor = rclpy.executors.MultiThreadedExecutor()
...
#            print("Robot path  ",pixellate_map_obj.path)
            ...
            if k == 2:
                list_of_path = [map_coord(y, x, grid_map_obj) for x, y in zip(*path_node.pixellate_map_obj.path)]  
            ...
            if k > 2:
                ...
                list_of_path = [map_coord(y, x, grid_map_obj) for x, y in zip(*path_node.pixellate_map_obj.path)]  
                list_of_angle = path_node.pixellate_map_obj.calculate_angle(list_of_path)
