...
def robot_drive( pid_linear, odom_raw_data, velocity_publisher, rate, laser_raw_data):
    ...
    while True:  
        ...
        if distance_to_goal < pid_linear.arrive_distance or abs(odom_raw_data.pose[2] - list_of_angle[i]) > 0.9:
            angularController = PID_angular(Kp = 8.0, Ki =0.03, Kd=0.0, dt=0.1,  desired_w=2.0 )
...
def main(args=None):
    ...
    rate = laser_raw_data.create_rate(10)
    
    while laser_raw_data.lidar_buf == 0:
        rate.sleep()
    
    global end_of_robot_position
    end_of_robot_position=[40, 40]
    ...
    print(pixellate_map_obj.path_map)
    i = 0

    pid_linear = PID_linear(Kp=7.0, Ki=0.5, Kd=0.05, dt=0.1, arrive_distance=0.1, angle_distance=0.1, desiredV=0.1)
    pid_linear.current = [odom_raw_data.pose[0], odom_raw_data.pose[1], odom_raw_data.pose[2]]
    ...
    try:
        while rclpy.ok(): 
            ...
            position = [int(odom_raw_data.pose[0]), int(odom_raw_data.pose[1]), int(odom_raw_data.pose[2])]
