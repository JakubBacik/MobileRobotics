...
class map:
    ...
    def iterateLidar(self, dataScan, robot_position):
        ...
        t1 = Thread( target = self.lidar_thread, args = ( dataScan, theta, sensor_position, 0, 127))
        t1.start()
        t2 = Thread( target = self.lidar_thread, args = ( dataScan, theta, sensor_position, 128, 255))
        t2.start()
        t3 = Thread( target = self.lidar_thread, args = ( dataScan, theta, sensor_position, 256, 383))
        t3.start()
        
        for k in range( 384, len(dataScan)): 
            if pl.isinf(dataScan[k]) or pl.isnan(dataScan[k]):
                continue  

            pt = pol2Car(dataScan[k], theta[k], sensor_position)

            buf_x, buf_y = self.ray_trace( pt, sensor_position)
            ...
            for i in range(len(buf_x)):
                self.map[buf_y[i]][buf_x[i]] = self.miss( self.map[buf_y[i]][buf_x[i]] )
            mutex.release()
    ...
    def lidar_thread( self, data_scan, theta, sens_pos, start, end):
        for k in range( start, end ):
            if pl.isinf(data_scan[k]) or pl.isnan(data_scan[k]):
                continue  

            pt = pol2Car(data_scan[k], theta[k], sens_pos)

            buf_x, buf_y = self.ray_trace( pt, sens_pos)
